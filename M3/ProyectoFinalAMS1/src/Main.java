...
/* ALEX:Solucionar bug lucha(Ensennar a Alex), Poner un icono, Solucionar verga de las armas, Poner label de el record
 */
						...
						if (player2.getLife() > 0) {
							...
							if (player2.getAgil() * 10 > atackPlayer2) {
								if (player1.getAgil() < dodgePlayer1) {
									...
									//
									System.out.println(bot.Username + ": HP of " + player1.getName() + " has "
											+ player1.getLife());
								...
								System.out.println(player1.getUsername() + ": Failed attack!");
							}
						} else {
							System.out.println(player2.getUsername() + ": Dodged attack!");
						}
					} else {
						System.out.println(player1.getUsername() + ": Failed attack!");
					}
				...
				if (player2.getLife() > 0) {
					...
					if (player2.getAgil() * 10 > atackPlayer2) {
						if (player1.getAgil() < dodgePlayer1) {
							...
							System.out.println(player1.getUsername() + ": Dodged attack!");
						}
					} else {
						System.out.println(player2.getUsername() + ": Failed attack!");
					}
					if (player2.getLife() < 1) {
						System.out.println(player2.getName() + " has been defeated.");
						panel4.getTerminal().setBackground(Color.GREEN);
				...
				if (player1.getUsername() == "CPU") {
					...
					// When you win:
						...
						ConnectionDB.insertBattle(warriorPlayer.getWarrior_name(), weaponPlayer.getWeapon_id(),
								...
								player2.getLife());
						int confirmado = JOptionPane.showConfirmDialog(null,
								"You have won, do you want to play another round?");
						if (JOptionPane.OK_OPTION == confirmado) {
							...
							Main.weaponCPU = weapons2
									.get((int) Math.floor(Math.random() * (weapons2.size() - 2 + 1) + 1));
							...
							setSize(945, 800);
							setLocationRelativeTo(null);
							panel4 = new Battle_Panel(panel1.getUsername().getText(), Main.warriorPlayer,
									Main.weaponPlayer, Main.warriorCPU, Main.weaponCPU);
		...
		public void actionPerformed(ActionEvent e) {
			...
			if ((e.getActionCommand().equals("Play") || e.getActionCommand().equals("Choose Warrior"))
				...
				if (panel1.getUsername().getText().length() > 15) {
					JOptionPane.showMessageDialog(null, "To much characters in Username, max 15", "Error",
